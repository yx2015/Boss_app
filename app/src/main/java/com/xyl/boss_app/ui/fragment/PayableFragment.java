...
import com.xyl.boss_app.ui.holder.PayableHolder;
 ...
 */
public class PayableFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, LoadMoreListView.OnLoadMoreListener {
    private static final String REQUEST_TAG_PAYABLE_FRAGMENT = PayableFragment.class.getSimpleName();
    ...
    private void refreshStatisticalView(FeeDto.FeeDtoInfo data) {
        synchronized (PayableFragment.class) {
            if (isCanShowingView) {
                if (null != mSwipeRefreshLayout) {
                    mSwipeRefreshLayout.setRefreshing(false);
                }
        ...
        if (data != null) {
            if (data.getTotalPayableMoney() == 0 && data.getTotalPrepaidMoney() == 0) {
                mListView.setVisibility(View.INVISIBLE);
                mFee.setVisibility(View.GONE);
                ...
                mCount.setText(data.getTotalPayableMoney() < 0 ? mFormat.format(data.getTotalPayableMoney()) : "-" + mFormat.format(data.getTotalPayableMoney()));
                mActualCount.setText("实付" + mFormat.format(data.getTotalPrepaidMoney()));
            }
        }
    }
    ...
    private void requestListData() {
        feeParams.setPageNo(pageNo);
        feeParams.setFeeType("2");
        VolleyHelper.getInstance().getRequestQueue().cancelAll(REQUEST_TAG_PAYABLE_FRAGMENT);
        NetWorkRequest request = new NetWorkRequest(UrlConstant.GET_FEE_LIST, feeParams, FeeDto.class,
                new Response.Listener<FeeDto>() {
                    @Override
                    public void onResponse(FeeDto feeStateDto) {
                        if ("200".equals(feeStateDto.getCode())) {
                            refreshListView(feeStateDto.getData());
                        } else {
                            UIUtils.showToast(feeStateDto.getMsg());
                        }
                    }
        ...
        VolleyHelper.getInstance().getRequestQueue().add(request).setTag(REQUEST_TAG_PAYABLE_FRAGMENT);
    }
     ...
     * @param feeStateInfo
     */
    private void refreshListView(final FeeDto.FeeDtoInfo feeStateInfo) {
        synchronized (PayableFragment.class) {
            if (isCanShowingView) {
                if (null != mSwipeRefreshLayout) {
                    mSwipeRefreshLayout.setRefreshing(false);
    ...
    public void onPageSelected(boolean isNeedRefresh, FeeParams params) {
        ...
        if (!this.isNeedRefresh) {
            ...
            if ("2".equals(feeParams.getFeeType()) && isNeedRefresh) {
                requestDataFromNet();
